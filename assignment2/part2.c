...
int main(int argc, char *argv[]){
	...
	pid_t pid[10];
	FILE *fp;
	int count=0, i, j=0, wstatus, countArg = 0, a=0;
	...
	if(strstr(argv[1], ".txt")){
		...
		printf("sending signal to pause all the processes!\n");
    	singler(j, SIGUSR1, pid);
   		sleep(3);
   		printf("sending signal to restart all the processes!\n");
   		singler(j, SIGUSR1, pid);
   		sleep(3);
   		printf("sending signal to stop all the processes!\n");
   		singler(j, SIGSTOP, pid);
   		sleep(4);
   		printf("sending signal to continue all the processes!\n");
   		singler(j, SIGCONT, pid);
		sleep(3);
		printf("terminating all the processes!\n");
		singler(j, SIGINT, pid);
		sleep(3);
