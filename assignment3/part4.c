...
int getEntry(struct topicQueue *tpQueue, int lastEntry, struct topicEntry *temp){
	if(tpQueue->head == tpQueue->tail || lastEntry == tpQueue->head 				// return 0 if the queue is empty 
			|| lastEntry == 0){														// or the entry has been deleted
		return 0;																	// or the entry is the newest
	}
	z+=1;
	for (int j = tpQueue->tail; j < tpQueue->head; j++){							// copy last entry +1 data to the temp buffer 
		if (tpQueue->entry[j].entryNum == lastEntry){								// if the last entry is in the queue
			...
			return 1;																// return 1 for the success
...
int cleanUp(struct topicEntry *temp, struct topicQueue *tpQueue){
	gettimeofday(&temp->timeStamp,NULL);											// get time stamp, and compare it with each entry's delta
	int diff, result;
	...
	for (int j = 0; j < tpQueue->head; ++j){
		...
		if (diff > delta){															// call dequeue function if the entry is too old
...
void* subscriber(void* arg){
	...
	for (int i = 0; i < MAXENTRIES; ++i){
		if (sub[i].thread_id == pthread_self()){
			sub[i].flag = 0;														// change the subscriber thread flag
			break;
		}
	}
	return NULL;
...
void* pthread_cleanUp(void* arg){
	...
	while(check == 1){
		...
		pthread_mutex_unlock(&lock[threadCle->lockPos]);							// unlock the clean up critical section
...
int main(int argc, char* argv[]){
		...
		while( (line = getline(&text, &size, fp)) != -1){
			...
			if(strcmp(command[0], "add") == 0){										// "add publisher/subscriber" command
				...
				if (strcmp(command[1], "publisher") == 0){							
					...
					if (strlen(pubFilename) <= 1 || !(strstr(pubFilename,".txt") ) ){
						...
						for(i=0; i<MAXENTRIES; i++){
							if(pub[i].flag == 0){
								for(j=0; j<pub_cnt; j++){							// create thread for publisher
					...
					actSub += 1;													// varible that track act-subscriber
					subFile += 1;
			...
			if(strcmp(command[0], "start\n") == 0 || strcmp(command[0], "start") == 0){	// "start" command
				for (int k = 0; k < MAXENTRIES; ++k){	
					if (pub[k].flag == 1){
						pthread_mutex_lock(&condition_mutex);
						pthread_cond_broadcast(&cond);								// join publiser thread
						pthread_mutex_unlock(&condition_mutex);
				...
				for (int k = 0; k < MAXENTRIES; ++k){	
					if (pub[k].flag == 1){
						pthread_mutex_lock(&condition_mutex);
						pthread_cond_broadcast(&cond);								// join subscriber thread
						pthread_mutex_unlock(&condition_mutex);
				...
				for (int i = 0; i < actQueue; ++i){
                	threadCle[i].regisNum = i;										// create clean up thread
				...
				printf("-------------------------------------------------------------------------\n");
				pthread_mutex_unlock(&condition_mutex);								// join clean up thread
	...
	sleep(1);
	int o = 0, v=0;
	for(i=0; i<actSub; i++){														// write data to subscriber html file
		x += 1;
